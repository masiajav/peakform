...
const ENABLED_FIELD: Record<Exclude<OrderTier, 'trial'>, keyof Expert> = {
...
export default function TierSelector({ expert, hasUsedTrial, isLoggedIn, stripeConnected }: Props) {
  const enabledTiers = useMemo(
    () => STANDARD_TIERS.filter(tier => expert[ENABLED_FIELD[tier]] !== false),
    [expert],
  )
  const defaultTier: OrderTier = expert.trial_enabled && !hasUsedTrial
    ? 'trial'
    : enabledTiers.includes('pro') ? 'pro' : enabledTiers[0] ?? 'starter'
  const [selected, setSelected] = useState<OrderTier>(defaultTier)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const selectedStandardDisabled = selected !== 'trial' && !enabledTiers.includes(selected)
  const currentSelection: OrderTier = selectedStandardDisabled ? enabledTiers[0] ?? 'trial' : selected
  const basePrice = currentSelection === 'trial'
    ? (expert.trial_price ?? 0)
    : (expert[PRICE_FIELD[currentSelection]] as number)
  ...
  if (enabledTiers.length === 0 && (!expert.trial_enabled || hasUsedTrial)) {
    return (
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', padding: '18px 20px' }}>
        <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 22, color: 'var(--text)', margin: '0 0 8px', letterSpacing: 1 }}>
          SIN TIERS DISPONIBLES
        </h2>
        <p style={{ color: 'var(--text2)', fontSize: 13, lineHeight: 1.6, margin: 0 }}>
          Este experto esta ajustando sus ofertas. Vuelve pronto.
        </p>
      </div>
    )
  }
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
        {expert.trial_enabled && (
          <button
            ...
            style={{
              background: currentSelection === 'trial' ? 'rgba(255,214,0,0.06)' : 'var(--surface)',
              border: `1px solid ${currentSelection === 'trial' ? 'var(--yellow)' : 'var(--border)'}`,
              padding: '20px 24px',
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <span style={{
                    ...
                    color: currentSelection === 'trial' ? 'var(--yellow)' : 'var(--text)', letterSpacing: 0.5,
                  }}>
                    PRUEBA
                  </span>
                  <span style={{
                    ...
                    border: '1px solid rgba(255,214,0,0.3)', padding: '1px 7px',
                  }}>
                    SOLO 1 VEZ
                  </span>
                  {expert.trial_refundable && (
                    <span style={{
                      ...
                      border: '1px solid rgba(0,214,127,0.3)', padding: '1px 7px',
                    }}>
                      REEMBOLSO 7D
                    </span>
                  )}
              ...
              <div style={{
                ...
                color: currentSelection === 'trial' ? 'var(--yellow)' : 'var(--text)',
              }}>
        ...
        {enabledTiers.map(tier => {
          ...
          const isSelected = currentSelection === tier
                      ...
                      <ul style={{ margin: '10px 0 0', padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {cfg.features.map(f => (
                          <li key={f} style={{ fontSize: 12, color: 'var(--text2)', display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ color: 'var(--green)', fontSize: 10 }}>OK</span> {f}
                          </li>
                        ))}
                      </ul>
                    </>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: 'var(--text2)', marginBottom: 10 }}>
          <span>Comision plataforma (20%)</span>
      ...
      {!stripeConnected ? (
        ...
        <>
          <button
            ...
            style={{ fontSize: 16 }}
          >
            {loading ? 'REDIRIGIENDO...' : `COMPRAR ${TIER_CONFIG[currentSelection].label.toUpperCase()} ->`}
          </button>
